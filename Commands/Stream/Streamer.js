...
exports.run = async (client, message, args, prefix) => {
  if (args[0]) {
    ...
    if (jsonManager.readJson() != null && jsonManager.readJson() != {}) {
      streamerIDList = Object.keys(jsonManager.readJson())
      streamerNameList = Object.values(jsonManager.readJson()).map(item => item.name)
    }
    try {
      if (streamerIDList != null) {
        const channelManager = new JsonManager(`./Data/channel.json`);
        if (!channelManager.readJson().hasOwnProperty(message.guild.id)) {
        ...
        switch (args[0]) {
          ...
          case "추가":
            ...
            const liveResponse = await axios.get(`https://api.chzzk.naver.com/polling/v2/channels/${channelId}/live-status`)
            var liveData = liveResponse.data;
            const forumChannelId = channelManager.readJson()[message.guild.id]
          ...
          case "삭제":
            ...
            if (streamerIDList.includes(delStreamer)) {
              message.guild.channels.resolve(delStreamer)
              jsonManager.removeField(delStreamer)
            } else if (streamerNameList.includes(delStreamer)) {
              streamerNameList.shift(delStreamer)
              for (const key in jsonManager.readJson()) {
                if (jsonManager.readJson()[key].name == delStreamer) {
                  message.guild.channels.resolve(jsonManager.readJson()[key].chattingChannel).delete(`<@${message.member.id}>`)
          ...
          case "cs":
            ...
            if (notiChannel) {
              channelManager.updateField(serverId, args[1].match(/\d+/g)[0])
